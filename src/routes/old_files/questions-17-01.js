...
router.post('/questions/new-question', isAuthenticated, async (req, res) => {
  ...
  if (!reward || reward > 30 || reward < 3) {
    errors.push({ text: 'Please Add a valid Reward (greater than 3 USD and less than 30 USD)' })
  }
  if (!tagsArray) {
...
router.get('/questions/allquestions/:skip?', async (req, res) => {
...
router.post('/questions/allquestionsfilter/:skip?', async (req, res) => {
...
router.put('/questions/doanswer/:id', isAuthenticated, async (req, res) => {
  ...
  if (errors.length > 0) {
    ...
    const newAnswer = new Answer(
      {
        answer, user_answer, user_question, id_question, best_answer: false, get_paid: false, withdrawal_requested: false,
  ...
  res.render('questions/myanswers', {
    ...
    helpers: {
      ...
      ButtonWEnabled: function (best_answer, get_paid, withdrawal_requested, options) {
        ...
        if (best_answer && !get_paid && !withdrawal) { enabled = true; }
        return (enabled == true) ? options.fn(this) : options.inverse(this);
      },
      SuccesfullyWithdrawn: function (best_answer, get_paid, withdrawal_requested, options) {
        var enabled = false;
        var withdrawal;
        if (withdrawal_requested == null) { withdrawal = false; } else { withdrawal = withdrawal_requested; }
        if (best_answer && get_paid && withdrawal) { enabled = true; }
      ...
      WithdrawalRequested: function (best_answer, get_paid, withdrawal_requested, options) {
        var enabled = false;
        var withdrawal;
        if (withdrawal_requested == null) { withdrawal = false; } else { withdrawal = withdrawal_requested; }
        if (best_answer && !get_paid && withdrawal) { enabled = true; }
      ...
      NotBestAnswer: function (variable1, options) {
        return (variable1 == false) ? options.fn(this) : options.inverse(this);
      },
      ifEquals: function (variable1, variable2, options) {
        return (variable1.toString() == variable2.toString()) ? options.fn(this) : options.inverse(this);
      },
...
router.get('/questions/withdraw_answer', isAuthenticated, async (req, res) => {
...
///// Solicitar retiro de dinero (es cuando el que hace la pregunta se gana la recompensa y desea retirar el dinero en bitcoins) ///////
router.put('/questions/withdraw_answer', isAuthenticated, async (req, res) => {
  ...
  const errors = [];
  if (!wallet_address) {
    errors.push({ text: 'Please Write a Wallet Address' });
  }
  if (errors.length > 0) {
    res.render('questions/myanswers', { question }, {
      errors,
      //title,
      // description
    })
    ...
    res.redirect('/questions/myanswers')
